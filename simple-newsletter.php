...
if(!class_exists('simpleNewsletter')) {
	...
	class simpleNewsletter {
		...
		{
			if( !isset($_GET['page']) || !in_array($_GET['page'], array('simplenewsletter-grid', 'simplenewsletter-grid-mobile', 'simplenewsletter-admin')) ){
				return ;
			}
			
			wp_enqueue_script( 'simplenewsletter-admin', plugins_url('js/admin_main.js', __FILE__), array('jquery'));
			wp_enqueue_style( 'simplenewsletter-admin-css', plugins_url('css/admin.css', __FILE__));
		}
